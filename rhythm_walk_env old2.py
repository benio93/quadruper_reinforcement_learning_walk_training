...
class RhythmWalkEnv(QuadEnv):
    ...
    def step(self, action):
        ...
        # Nowa forma nagrody za zakres
        joint_range_reward = self.full_joint_bonus * avg_delta * 2.0

        # Kara za drżenie (brak odważnych ruchów)
        tremble_penalty = 0.0
        if max_delta < 0.1:
            tremble_penalty = 1

        # Bonus za porządne wychylenie
        strong_kick_bonus = 0.0
        if max_delta > 0.25:
            strong_kick_bonus = 1
