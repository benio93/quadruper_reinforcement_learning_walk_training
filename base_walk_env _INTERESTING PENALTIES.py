...
class BaseWalkEnv(QuadEnv):
    def __init__(self, use_gui=False, max_episode_steps=2000):
        ...
        self.forward_bonus              = 1.5     # nagroda za prędkość w osi X
        ...
        self.max_lateral_dev            = 0.35    # offtrack > 0.5 m od osi Y
        ...
        # shaping bonus za zbliżanie się do mety
        self.distance_bonus_weight = 0.1
        # kara za “lot” – gdy żadna stopa nie dotyka podłoża
        self.air_penalty = 1.0 


        # Inne stałe środowiska
        self.finish_line_x              = 1.0
        self.too_high_limit             = 0.27
        self.too_low_limit              = 0.05
        self.tilt_limit                 = 45
        self.step_counter               = 0

        # indeksy linków końcówek nóg (tibia)
        # znajdź te jointy, których linkName zawiera "tibia"
        # w BaseWalkEnv.reset(), jako pierwsze linijki po super().reset():
    ...
    def reset(self):
        ...
        print("Foot indices:", self.foot_indices)  # możesz później usunąć ten print
    ...
    def step(self, action):
        ...
        # kontaktowy bonus: premiuj kroki, gdy co najmniej 2 stopy trzymają podłogę
        contacts = [
            len(self.p.getContactPoints(bodyA=self.robot_id, linkIndexA=li)) > 0
            for li in self.foot_indices
        ]
        cnt = sum(contacts)
        # print(f"contacts: {contacts} | sum={cnt} | tak? {cnt>=2}")
        if cnt >= 2:
            reward += 0.5

        # kara za to, że jest w powietrzu (żadna stopa nie dotyka ziemi)
        if cnt == 0:
