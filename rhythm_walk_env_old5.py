...
class RhythmWalkEnv(QuadEnv):
    def __init__(self,
        ...
        self.foot_indices = [0, 1, 2, 3]  # Przykładowe indeksy stóp – dostosuj do swojego modelu
    ...
    def reset(self):
        ...
        self.total_joint_distance = 0.0
        self.total_joint_direction_changes = 0
        _, _, yaw = self.p.getEulerFromQuaternion(self.p.getBasePositionAndOrientation(self.robot_id)[1])
        self.prev_yaw = yaw
        self.total_yaw_change = 0.0
        self._spawn_target()
        self.step_counter = 0
    ...
    def step(self, action):
        ...
        if avg_delta > 0.5:
            ...
            joint_range_reward = 3.0
        elif avg_delta > 0.2:
            joint_range_reward = 1.5
        else:
            joint_range_reward = 0.0

        tremble_penalty = 1.0 if max_delta < 0.1 else 0.0
        strong_kick_bonus = 1.0 if max_delta > 0.25 else 0.0
        ...
        heading_reward = self.move_bonus * max(cos_err, 0.0) * 4
        ...
        # Air penalty if all feet are in the air
        ...
        air_penalty = 8.0 if feet_on_ground == 0 else 0.0
        reward -= air_penalty
